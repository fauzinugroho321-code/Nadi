...
import Link from "next/link"; // <-- Tambahan
...
export default function EmployeeDashboard() {
  ...
  const [workType, setWorkType] = useState("WFO"); // <-- State Baru

  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const [record, dashboardData] = await Promise.all([ getTodayAttendance(), getEmployeeDashboardData() ]);
        ...
        if (record && record.clockIn) {
          ...
          if (!record.clockOut) {
            ...
            setClockedIn(false); setHasClockedOut(true);
        ...
        setWorkType(dashboardData.workType); // <-- Simpan tipe kerja
  ...
  const handleClockToggle = async () => {
    ...
    try {
      ...
      if (!clockedIn) { setClockedIn(true); setElapsed(0); } 
      else { setClockedIn(false); setHasClockedOut(true); }
  ...
  if (loading) return <div className="h-64 flex flex-col items-center justify-center gap-3 text-indigo-400"><RefreshCw className="animate-spin" /><span className="text-sm font-medium animate-pulse">Menyiapkan Dashboard...</span></div>;
      ...
      <section className={`relative rounded-3xl overflow-hidden p-10 transition-all duration-700 ${clockedIn ? 'bg-gradient-to-br from-[#111527] to-[#141130] border-indigo-500/30 shadow-[0_0_60px_rgba(99,102,241,0.1)]' : 'bg-gradient-to-br from-[#111527] to-[#0F1223] border-white/5'}`}>
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex flex-col items-center gap-6">
             ...
             <button onClick={handleClockToggle} disabled={actionLoading || hasClockedOut} className={`w-32 h-32 rounded-full flex flex-col items-center justify-center gap-2 transition-all duration-300 disabled:opacity-50 disabled:scale-100 ${clockedIn ? 'bg-indigo-500/20 border-2 border-indigo-400/50 shadow-[0_0_40px_rgba(99,102,241,0.3)]' : 'bg-gradient-to-br from-indigo-500 to-violet-600 border-2 border-white/10 shadow-2xl hover:scale-105'}`}>
               {actionLoading ? <RefreshCw size={24} className="text-white animate-spin" /> : clockedIn ? <LogOut size={24} className="text-indigo-200" /> : <LogIn size={24} className="text-white" />}
               <span className="text-xs font-bold text-white tracking-wide">{hasClockedOut ? "Done for Today" : clockedIn ? "Clock Out" : "Clock In"}</span>
             </button>
             {/* DINAMIS: Label WFO/WFH sesuai Database */}
             <div className="px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs text-emerald-300 flex items-center gap-2">
               <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"/> Working {workType === 'WFO' ? 'from Office' : workType === 'WFH' ? 'from Home' : 'Hybrid'}
             </div>
          </div>
          <div className="flex flex-col items-center md:items-end flex-1">
             ...
             <p className="text-sm text-slate-400 mt-2">{hasClockedOut ? "Total work time today" : clockedIn ? "Today's work time" : "Start your session"}</p>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#111527]/70 border border-indigo-500/10 rounded-2xl p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            {/* DINAMIS: Navigasi View All */}
            <Link href="/employee/tasks" className="text-xs text-indigo-400 flex items-center hover:text-indigo-300 transition">View all <ChevronRight size={14}/></Link>
          </div>
          <div className="space-y-4 flex-1">
             {tasks.length > 0 ? (
               tasks.map((task) => (
                 <div key={task.id} className="flex items-start gap-3 group">
                   {task.status === "IN_PROGRESS" ? <RefreshCw size={16} className="text-amber-400 shrink-0 mt-0.5 animate-[spin_3s_linear_infinite]" /> : <Circle size={16} className="text-slate-500 shrink-0 mt-0.5" />}
                   <div>
                     <p className="text-sm text-slate-200 group-hover:text-indigo-300 transition-colors cursor-pointer">{task.title}</p>
                     {task.dueDate && <p className="text-[11px] text-slate-500 flex items-center gap-1 mt-1"><Clock size={10} /> Due {format(new Date(task.dueDate as any), "MMM dd")}</p>}
                   </div>
                 </div>
               ))
             ) : (
